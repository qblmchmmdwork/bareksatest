...
class DanaKelolaanRepository(private val remoteDataSource: DanaKelolaanRemoteDataSource) :
    GrafikDataProvider {
    suspend fun getSinceLastYear(id: String) : RepositoryResource<List<DanaKelolaan>> {
        return when (val res = remoteDataSource.getDanaKelolaanByIdSince(id, 1, TimeUnit.Year)) {
    ...
    override suspend fun getGrafikDataSinceLastYear(id: String): RepositoryResource<List<GrafikFragment.GrafikEntry>> {
        return when (val res = getSinceLastYear(id)) {
